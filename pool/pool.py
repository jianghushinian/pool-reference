...
class Pool:
    ...
    async def calculate_24_hours_pool_capacity(self, unit: str) -> Dict:
        """
        ...
        timestamp = uint64(int(yesterday.timestamp()))
        units: Dict = {
            "pib": 5,
            "tib": 4,
        }
        ...
        for launcher_id, point in each_farmer_points.items():
            each_farmer_capacity[launcher_id] = float(point) / (86400 * 1.088e-15) / (1024 ** units[unit])
